...
INTF_CONFIG_STUDIO_ID = "bf40ee48-7fa0-4347-a7b4-5a8d794943e7"
...
def get_studio_inputs(client, studio_id, workspace_id):
    '''
    Returns the inputs for a given studio in a workspace.
    '''
    req = studio_services.InputsServiceStub().GetOne(
        studio_models.InputsKey(
            workspace_id=wrappers.StringValue(value=workspace_id),
            studio_id=wrappers.StringValue(value=studio_id)
        ),
        timeout=RPC_TIMEOUT
    )
    stub = studio_services.InputsConfigServiceStub(client)
    res = stub.GetOne(req, timeout=RPC_TIMEOUT)
    return res.value.inputs.value
...
def update_intf_config_studio(channel, workspace_id, inputs, dev_ids, path_values=[]):
    '''
    ...
    req = studio_services.InputsConfigSetRequest(
        value=studio_models.InputsConfig(
            key=studio_models.InputsKey(
                ...
                path=fmp_wrappers.RepeatedString(values=path_values)
            ),
